...
IsWindowHovered(ImGuiHoveredFlags flags =
                    0); // is current window hovered (and typically: not blocked by a popup/modal)? see flags for
...
IMGUI_API void PushTabStop(bool tab_stop); // == tab stop enable. Allow focusing using TAB/Shift-TAB, enabled by default
                                           // but you can disable it for certain widgets
IMGUI_API void PopTabStop();
IMGUI_API void
PushButtonRepeat(bool repeat); // in 'repeat' mode, Button*() functions return repeated true in a typematic manner
                               // (using io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that you can call
                               // IsItemActive() after any Button() to tell if the button is held in the current frame.
...
IMGUI_API const char *
GetKeyName(ImGuiKey key); // [DEBUG] returns English name of the key. Those names a provided for debugging purpose and
...
enum ImGuiWindowFlags_ {
  ...
  ImGuiWindowFlags_UnsavedDocument =
      1 << 20, // Display a dot next to the title. When used in a tab/docking context, tab is selected when clicking the
               // X + closure is not assumed (will wait for user to stop submitting the tab). Otherwise closure is
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_EnterReturnsTrue =
      1 << 5, // Return 'true' when Enter is pressed (as opposed to every time the value was modified). Consider looking
              // at the IsItemDeactivatedAfterEdit() function.
...
enum ImGuiTreeNodeFlags_ {
  ...
  ImGuiTreeNodeFlags_NavLeftJumpsBackHere = 1 << 14, // (WIP) Nav: left direction may move to this TreeNode() from any
...
enum ImGuiPopupFlags_ {
  ...
  ImGuiPopupFlags_MouseButtonMiddle = 2, // For BeginPopupContext*(): open on Middle Mouse release. Guaranteed to always
...
enum ImGuiTabBarFlags_ {
  ...
  ImGuiTabBarFlags_NoCloseWithMiddleMouseButton =
      1 << 3, // Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You
              // can still repro this behavior on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open =
              // false.
...
enum ImGuiTabItemFlags_ {
  ...
  ImGuiTabItemFlags_NoCloseWithMiddleMouseButton =
      1 << 2, // Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You
              // can still repro this behavior on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open =
              // false.
...
enum ImGuiTableFlags_ {
  ...
  ImGuiTableFlags_RowBg = 1 << 6, // Set each RowBg color with ImGuiCol_TableRowBg or ImGuiCol_TableRowBgAlt (equivalent
  ...
  ImGuiTableFlags_PreciseWidths =
      1 << 19, // Disable distributing remainder width to stretched columns (width allocation on a 100-wide table with 3
               // columns: Without this flag: 33,33,34. With this flag: 33,33,33). With larger number of columns,
  ...
  ImGuiTableFlags_NoPadInnerX = 1 << 23, // Disable inner padding between columns (double inner padding if BordersOuterV
                                         // is on, single inner padding if BordersOuterV is off). Scrolling
...
enum ImGuiHoveredFlags_ {
  ...
  ImGuiHoveredFlags_AllowWhenOverlappedByItem = 1
  ...
  // - this is a shortcut to pull flags from 'style.HoverFlagsForTooltipMouse' or 'style.HoverFlagsForTooltipNav' where
  // you can reconfigure desired behavior.
...
enum ImGuiDockNodeFlags_ {
  ...
  // ImGuiDockNodeFlags_NoCentralNode              = 1 << 1,   //       // Disable Central Node (the node which can stay
  // empty)
  ImGuiDockNodeFlags_NoDockingOverCentralNode =
      1 << 2, //       // Disable docking over the Central Node, which will be always kept empty.
  ImGuiDockNodeFlags_PassthruCentralNode =
      1 << 3, //       // Enable passthru dockspace: 1) DockSpace() will render a ImGuiCol_WindowBg background covering
...
enum ImGuiKey : int {
  ...
  ImGuiKey_GamepadDpadDown,  // D-pad Down                                       // Move / Tweak / Resize Window (in
                             // Windowing mode)
  ImGuiKey_GamepadL1, // L Bumper (Xbox)  L (Switch)   L1 (PS)            // Tweak Slower / Focus Previous (in Windowing
                      // mode)
...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_DpiEnableScaleViewports =
      1 << 14, // [BETA: Don't use] FIXME-DPI: Reposition and resize imgui windows when the DpiScale of a viewport
               // changed (mostly useful for the main viewport hosting other window). Note that resizing the main window
               // itself is up to your application.
...
enum ImGuiBackendFlags_ {
  ...
  ImGuiBackendFlags_HasSetMousePos =
      1 << 2, // Backend Platform supports io.WantSetMousePos requests to reposition the OS mouse position (only used if
...
struct ImGuiStyle {
  ...
  float WindowRounding; // Radius of window corners rounding. Set to 0.0f to have rectangular windows. Large values tend
                        // to lead to variety of artifacts and are not recommended.
  ...
  float TabMinWidthForCloseButton; // Minimum width for close button to appear on an unselected tab when hovered. Set to
                                   // 0.0f to always show when hovering, set to FLT_MAX to never show close button
                                   // unless selected.
  ...
  bool AntiAliasedLinesUseTex; // Enable anti-aliased lines/borders using textures where possible. Require backend to
                               // render with bilinear filtering (NOT point/nearest filtering). Latched at the beginning
                               // of the frame (copied to ImDrawList).
  bool
      AntiAliasedFill; // Enable anti-aliased edges around filled shapes (rounded rectangles, circles, etc.). Disable if
...
struct ImGuiIO {
  ...
  const char *IniFilename; // = "imgui.ini"    // Path to .ini file (important: default "imgui.ini" is relative to
                           // current working dir!). Set NULL to disable automatic .ini loading/saving or if you want to
  ...
  ImVec2 DisplayFramebufferScale; // = (1, 1)         // For retina display or other situations where window coordinates
                                  // are different from framebuffer coordinates. This generally ends up in
  ...
  bool ConfigDockingAlwaysTabBar; // = false          // [BETA] [FIXME: This currently creates regression with
                                  ...
                                  // a docking node.
  bool
      ConfigDockingTransparentPayload; // = false          // [BETA] Make window or viewport transparent when docking
                                       // and only display docking boxes on the target viewport. Useful if rendering of
  ...
  bool ConfigInputTrickleEventQueue; // = true           // Enable input queue trickling: some types of events submitted
                                     // during the same frame (e.g. button down + up) will be spread over multiple
  ...
  // Presently Begin()/End() and BeginChild()/EndChild() needs to ALWAYS be called in tandem, regardless of return value
  // of BeginXXX() This is inconsistent with other BeginXXX functions and create confusion for many users. We expect to
  // update the API eventually. In the meanwhile we provide tools to facilitate checking user-code behavior.
  ...
  // Option to deactivate io.AddFocusEvent(false) handling. May facilitate interactions with a debugger when focus loss
  // leads to clearing inputs data. Backends may have other side-effects on focus loss, so this will reduce side-effects
  // but not necessary remove all of them. Consider using e.g. Win32's IsDebuggerPresent() as an additional filter (or
  // see ImOsIsDebuggerPresent() in imgui_test_engine/imgui_te_utils.cpp for a Unix compatible version).
  ...
  IMGUI_API void AddMousePosEvent(float x, float y); // Queue a mouse position update. Use -FLT_MAX,-FLT_MAX to signify
  ...
  IMGUI_API void AddMouseViewportEvent(
      ImGuiID id); // Queue a mouse hovered viewport. Requires backend to set ImGuiBackendFlags_HasMouseHoveredViewport
  ...
  bool WantCaptureKeyboard; // Set when Dear ImGui will use keyboard inputs, in this case do not dispatch them to your
                            // main game/application (either way, always pass keyboard inputs to imgui). (e.g. InputText
                            // active, or an imgui window is focused and navigation is enabled, etc.).
  bool WantTextInput; // Mobile/console: when set, you may display an on-screen keyboard. This is set by Dear ImGui when
  ...
  int KeyMap[ImGuiKey_COUNT]; // [LEGACY] Input: map of indices into the KeysDown[512] entries array which represent
                              ...
                              // Legacy backend will write into KeyMap[] using ImGuiKey_ indices which are always >512.
  ...
  float
      NavInputs[ImGuiNavInput_COUNT]; // [LEGACY] Since 1.88, NavInputs[] was removed. Backends from 1.60 to 1.86 won't
  ...
  ImGuiID MouseHoveredViewport; // (Optional) Modify using io.AddMouseViewportEvent(). With multi-viewports: viewport
                                ...
                                // io.BackendFlags |= ImGuiBackendFlags_HasMouseHoveredViewport if you can provide this
                                // info. If you don't imgui will infer the value using the rectangles and last focused
...
struct ImGuiWindowClass {
  ImGuiID ClassId; // User data. 0 = Default class (unclassed). Windows of different classes cannot be docked with each
                   // others.
  ImGuiID ParentViewportId; // Hint for the platform backend. -1: use default. 0: request platform backend to not parent
                            // the platform. != 0: request platform backend to create a parent<>child relationship
                            // between the platform windows. Not conforming backends are free to e.g. parent every
  ...
  ImGuiViewportFlags ViewportFlagsOverrideClear; // Viewport flags to clear when a window of this class owns a viewport.
...
struct ImGuiListClipper {
  ...
  // not be clipped, regardless of their visibility. (Due to alignment / padding of certain items it is possible that an
  // extra item may be included on either end of the display range).
...
struct ImDrawCmd {
  ...
  ImTextureID TextureId; // 4-8  // User-provided texture ID. Set by user in ImfontAtlas::SetTexID() for fonts or passed
                         // to Image*() functions. Ignore if never using images or multiple fonts atlas.
...
enum ImDrawFlags_ {
  ...
  ImDrawFlags_RoundCornersBottomRight =
      1 << 7, // AddRect(), AddRectFilled(), PathRect(): enable rounding bottom-right corner only (when rounding > 0.0f,
...
struct ImDrawList {
  ...
  // OBSOLETED in 1.80 (Jan 2021) inline  void  PathBezierCurveTo(const ImVec2& p2, const ImVec2& p3, const ImVec2& p4,
  // int num_segments = 0) { PathBezierCubicCurveTo(p2, p3, p4, num_segments); } // OBSOLETED in 1.80 (Jan 2021)
...
struct ImDrawData {
  ...
  ImVector<ImDrawList *> CmdLists; // Array of ImDrawList* to render. The ImDrawLists are owned by ImGuiContext and only
  ...
  IMGUI_API void
  ScaleClipRects(const ImVec2 &fb_scale); // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final
...
struct ImFontConfig {
  ...
  int OversampleH;  // 2        // Rasterize at higher quality for sub-pixel positioning. Note the difference between 2
                    // and 3 is minimal. You can reduce this to 1 for large glyphs save memory. Read
  ...
  const ImWchar *GlyphRanges; // NULL     // THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT IS ALIVE. Pointer to a
                              // user-provided list of Unicode range (2 value per range, values are inclusive,
...
struct ImFontGlyphRangesBuilder {
  ...
  IMGUI_API void
  AddRanges(const ImWchar *ranges); // Add ranges, e.g. builder.AddRanges(ImFontAtlas::GetGlyphRangesDefault()) to force
...
struct ImFont {
  ...
  ImU8 Used4kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 4096 /
                      8]; // 2 bytes if ImWchar=ImWchar16, 34 bytes if ImWchar==ImWchar32. Store 1-bit for each block of
                          // 4K codepoints that has one active glyph. This is mainly used to facilitate iterations
...
enum ImGuiViewportFlags_ {
  ...
  ImGuiViewportFlags_NoDecoration =
      1 << 3, // Platform Window: Disable platform decorations: title bar, borders, etc. (generally set all windows, but
              // if ImGuiConfigFlags_ViewportsDecoration is set we only set this on popups/tooltips)
  ...
  ImGuiViewportFlags_NoRendererClear =
      1
      << 8, // Platform Window: Renderer doesn't need to clear the framebuffer ahead (because we will fill it entirely).
...
struct ImGuiViewport {
  ...
  // fields, they are merely storage to facilitate backend implementation.
  ...
  void *PlatformHandle; // void* for FindViewportByPlatformHandle(). (e.g. suggested to use natural platform handle such
                        // as HWND, GLFWWindow*, SDL_Window*)
  void *PlatformHandleRaw; // void* to hold lower-level, platform-native window handle (under Win32 this is expected to
                           // be a HWND, unused for other platforms), when using an abstraction layer like GLFW or SDL
...
struct ImGuiPlatformIO {
  ...
  // handling), one for Renderer. Custom engine backends will often provide both Platform and Renderer interfaces and so
  // may not need to use all functions. Platform functions are typically called before their Renderer counterpart, apart
  // from Destroy which are called the other way.
...
IMGUI_API bool ImageButton(ImTextureID user_texture_id, const ImVec2 &size, const ImVec2 &uv0 = ImVec2(0, 0),
                           ...
                           const ImVec4 &tint_col = ImVec4(
                               1, 1, 1, 1)); // Use new ImageButton() signature (explicit item id, regular FramePadding)
...
typedef ImGuiKeyChord ImGuiModFlags; // == int. We generally use ImGuiKeyChord to mean "a ImGuiKey or-ed with any number
                                     // of ImGuiMod_XXX value", but you may store only mods in there.
enum ImGuiModFlags_ {
