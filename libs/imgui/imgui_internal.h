...
enum ImGuiItemFlags_ {
  ...
  ImGuiItemFlags_AllowOverlap = 1 << 9, // false     // Allow being overlapped by another widget. Not-hovered to Hovered
                                        // transition deferred by a frame.
...
enum ImGuiHoveredFlagsPrivate_ {
  ...
  ImGuiHoveredFlags_AllowedMaskForIsWindowHovered =
      ...
      ImGuiHoveredFlags_NoPopupHierarchy | ImGuiHoveredFlags_DockHierarchy | ImGuiHoveredFlags_AllowWhenBlockedByPopup |
...
enum ImGuiInputTextFlagsPrivate_ {
  ...
  ImGuiInputTextFlags_MergedItem = 1 << 28, // For internal use by TempInputText(), will skip calling ItemAdd(). Require
...
enum ImGuiButtonFlagsPrivate_ {
  ...
  ImGuiButtonFlags_NoNavFocus =
      1 << 18, // don't override navigation focus when activated (FIXME: this is essentially used everytime an item uses
  ...
  ImGuiButtonFlags_NoTestKeyOwner = 1 << 21, // don't test key/input owner when polling the key (note: mouse buttons are
                                             // keys! often, the key in question will be ImGuiKey_MouseLeft!)
...
enum ImGuiSelectableFlagsPrivate_ {
  ...
  ImGuiSelectableFlags_SelectOnNav = 1 << 21, // (WIP) Auto-select when moved into. This is not exposed in public API as
  ...
  ImGuiSelectableFlags_SpanAvailWidth =
      1 << 24, // Span all avail width even if we declared less for layout purpose. FIXME: We may be able to remove this
...
struct ImGuiNextItemData {
  ...
  ImGuiSelectionUserData SelectionUserData; // Set by SetNextItemSelectionUserData() (note that NULL/0 is a valid value,
                                            // we use -1 == ImGuiSelectionUserData_Invalid to mark invalid values)
  ImGuiCond OpenCond;
...
enum ImGuiInputFlags_ {
  ...
  ImGuiInputFlags_LockThisFrame = 1 << 6, // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will
...
enum ImGuiNavMoveFlags_ {
  ...
  ImGuiNavMoveFlags_FocusApi = 1 << 9, // Requests from focus API can land/focus/activate items even if they are marked
                                       // with _NoTabStop (see NavProcessItemForTabbingRequest() for details)
...
enum ImGuiTypingSelectFlags_ {
  ...
  ImGuiTypingSelectFlags_AllowBackspace =
      1 << 0, // Backspace to delete character inputs. If using: ensure GetTypingSelectRequest() is not called more than
              // once per frame (filter by e.g. focus state)
...
enum ImGuiDockNodeFlagsPrivate_ {
  ...
  ImGuiDockNodeFlags_CentralNode = 1 << 11, // Saved // The central node has 2 main properties: stay visible when empty,
  ...
  ImGuiDockNodeFlags_NoResizeY =
      1 << 17, //       //
               // Disable docking/undocking actions in this dockspace or individual node (existing docked nodes will be
               // preserved) Those are not exposed in public because the desirable sharing/inheriting/copy-flag-on-split
               // behaviors are quite difficult to design and understand. The two public flags
...
struct ImGuiViewportP : public ImGuiViewport {
  ...
  int LastFocusedStampCount; // Last stamp number from when a window hosted by this viewport was focused (by comparing
                             // this value between two viewport we have an implicit viewport z-order we use as fallback)
  ImGuiID LastNameHash;
  ImVec2 LastPos;
  float Alpha; // Window opacity (when dragging dockable windows/viewports we make them transparent)
  float LastAlpha;
  bool LastFocusedHadNavWindow; // Instead of maintaining a LastFocusedWindow (which may harder to correctly maintain),
...
struct ImGuiWindowSettings {
  ...
  short DockOrder; // Order of the last time the window was visible within its DockNode. This is used to reorder windows
                   // that are reappearing on the same frame. Same value between windows that were active and windows
                   // that were none are possible.
  bool Collapsed;
  bool
...
struct ImGuiContext {
  ...
  ImGuiWindow *WheelingWindow; // Track the window we started mouse-wheeling on. Until a timer elapse or mouse has
                               // moved, generally keep scrolling the same window even if during the course of scrolling
                               // the mouse ends up hovering a child window.
  ...
  int ViewportFocusedStampCount; // Every time the front-most window changes, we stamp its viewport with an incrementing
                                 // counter

  // Gamepad/keyboard Navigation
  ImGuiWindow *NavWindow;  // Focused window for navigation. Could be called 'FocusedWindow'
  ImGuiID NavId;           // Focused item for navigation
  ImGuiID NavFocusScopeId; // Identify a selection scope (selection code often wants to "clear other items" when landing
                           // on an item of the selection set)
  ...
  ImGuiSelectionUserData
      NavLastValidSelectionUserData; // Last valid data passed to SetNextItemSelectionUser(), or -1. For current window.
                                     // Not reset when focusing an item that doesn't have selection data.
  ...
  float HoverItemDelayClearTimer; // Currently used by IsItemHovered(): grace time before g.TooltipHoverTimer gets
                                  // cleared.
  ImGuiID HoverItemUnlockedStationaryId; // Mouse has once been stationary on this item. Only reset after departing the
                                         // item.
  ImGuiID HoverWindowUnlockedStationaryId; // Mouse has once been stationary on this window. Only reset after departing
                                           // the window.
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  ImU32 TreeJumpToParentOnPopMask; // Store a copy of !g.NavIdIsAlive for TreeDepth 0..31.. Could be turned into a ImU64
                                   // if necessary.
...
struct IMGUI_API ImGuiWindow {
  ...
  ImVec2 NavPreferredScoringPosRel[ImGuiNavLayer_COUNT]; // Preferred X/Y position updated when moving on a given axis,
                                                         // reset to FLT_MAX.
...
struct IMGUI_API ImGuiTable {
  ...
  ImRect OuterRect; // Note: for non-scrolling table, OuterRect.Max.y is often FLT_MAX until EndTable(), unless a height
...
IsKeyPressed(ImGuiKey key, ImGuiID owner_id,
             ImGuiInputFlags flags =
                 0); // Important: when transitioning from old to new IsKeyPressed(): old API has "bool repeat = true",
                     // so would default to repeat. New API requiress explicit ImGuiInputFlags_Repeat.
...
IMGUI_API bool
TreeNodeUpdateNextOpen(ImGuiID id, ImGuiTreeNodeFlags flags); // Return open state. Consume previous SetNextItemOpen()
