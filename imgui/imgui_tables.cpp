...
static const int TABLE_DRAW_CHANNEL_NOCLIP = 2; // When using ImGuiTableFlags_NoClip (this
                                                // becomes the last visible channel)
...
bool ImGui::BeginTableEx(const char *name, ImGuiID id, int columns_count, ImGuiTableFlags flags,
  ...
  PushOverrideID(id);
  if (instance_no > 0)
    PushOverrideID(instance_id); // FIXME: Somehow this is not resolved by stack-tool,
...
static void TableSetupColumnFlags(ImGuiTable *table, ImGuiTableColumn *column,
  ...
  if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0) {
    ...
    IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiTableColumnFlags_WidthMask_)); // Check that only 1 of
                                                                         // each set is used.
  }
...
void ImGui::TableUpdateLayout(ImGuiTable *table) {
  ...
  float sum_width_requests = 0.0f;  // Sum of all width for fixed and auto-resize columns,
                                    // excluding width contributed by Stretch columns but
                                    // including spacing/padding.
  ...
  table->BorderX1 = table->InnerClipRect.Min.x; // +((table->Flags & ImGuiTableFlags_BordersOuter) ?
                                                // 0.0f : -1.0f);
  table->BorderX2 = table->InnerClipRect.Max.x; // +((table->Flags & ImGuiTableFlags_BordersOuter) ?
                                                // 0.0f : +1.0f);
  ...
  // [Part 12] Sanitize and build sort specs before we have a change to use them
...
void ImGui::EndTable() {
  ...
  if (table->Flags & ImGuiTableFlags_NoHostExtendX) {
    ...
    outer_window->DC.IdealMaxPos.x =
        ImMax(outer_window->DC.IdealMaxPos.x, table->OuterRect.Min.x + table->ColumnsAutoFitWidth +
...
void ImGui::TableSetupColumn(const char *label, ImGuiTableColumnFlags flags,
  ...
  if (table->IsInitializing) {
    ...
    if (flags & ImGuiTableColumnFlags_DefaultSort &&
      ...
      column->SortOrder = 0; // Multiple columns using _DefaultSort will be reassigned
...
void ImGui::TableSetupScrollFreeze(int columns, int rows) {
  ...
  table->IsUnfrozenRows =
      (table->FreezeRowsCount == 0); // Make sure this is set before TableUpdateLayout()
                                     // so ImGuiListClipper can benefit from it.b
...
void ImGui::TableEndCell(ImGuiTable *table) {
  ...
  float *p_max_pos_x;
  if (table->RowFlags & ImGuiTableRowFlags_Headers)
    p_max_pos_x = &column->ContentMaxXHeadersUsed; // Useful in case user submit contents
...
void ImGui::TableSetColumnWidthAutoAll(ImGuiTable *table) {
  for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
    ImGuiTableColumn *column = &table->Columns[column_n];
    if (!column->IsEnabled &&
        !(column->Flags & ImGuiTableColumnFlags_WidthStretch)) // Cannot reset weight of
...
void ImGui::TableDrawBorders(ImGuiTable *table) {
  ...
  if (table->Flags & ImGuiTableFlags_BordersInnerV) {
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
      ...
      if (draw_y2 > draw_y1)
        inner_drawlist->AddLine(ImVec2(column->MaxX, draw_y1), ImVec2(column->MaxX, draw_y2), col,
                                border_size);
    }
  }
...
void ImGui::TableOpenContextMenu(int column_n) {
  ImGuiContext &g = *GImGui;
  ImGuiTable *table = g.CurrentTable;
  if (column_n == -1 && table->CurrentColumn != -1) // When called within a column automatically
...
void ImGui::DebugNodeTable(ImGuiTable *table) {
  ...
  const bool is_active =
      (table->LastFrameActive >=
       ImGui::GetFrameCount() - 2); // Note that fully clipped early out scrolling
...
void ImGui::EndColumns() {
  ...
  if (!(flags & ImGuiOldColumnFlags_GrowParentContentsSize))
    window->DC.CursorMaxPos.x = columns->HostCursorMaxPosX; // Restore cursor max pos, as
