...
//  ImGuiPlatformIO interface. 2023-02-15: Inputs: Use WM_NCMOUSEMOVE /
//  WM_NCMOUSELEAVE to track mouse position over non-client area (e.g. OS
//  decorations) when app is not focused. (#6045, #6162) 2023-02-02: Inputs:
//  Flipping WM_MOUSEHWHEEL (horizontal mouse-wheel) value to match other
//  backends and offer consistent horizontal scrolling direction. (#4019, #6096,
//  #1463) 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to
//  C++11. 2022-09-28: Inputs: Convert WM_CHAR values with MultiByteToWideChar()
//  when window class was registered as MBCS (not Unicode). 2022-09-26: Inputs:
//  Renamed ImGuiKey_ModXXX introduced in 1.87 to ImGuiMod_XXX (old names still
...
static void ImGui_ImplWin32_UpdateMouseData() {
  ...
  if (is_app_focused) {
    ...
    // When multi-viewports are enabled, all Dear ImGui positions are same as OS
    // positions.
...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport *viewport) {
  ...
  vd->Hwnd =
      ::CreateWindowEx(vd->DwExStyle, _T("ImGui Platform"), _T("Untitled"),
                       vd->DwStyle, // Style, class name, window name
                       rect.left, rect.top, rect.right - rect.left,
                       rect.bottom - rect.top, // Window area
                       parent_window, nullptr, ::GetModuleHandle(nullptr),
                       nullptr); // Parent window, Menu, Instance, Param
  vd->HwndOwned = true;
...
static void ImGui_ImplWin32_UpdateWindow(ImGuiViewport *viewport) {
  ...
  if (vd->DwStyle != new_style || vd->DwExStyle != new_ex_style) {
    ...
    HWND insert_after =
        top_most_changed
            ? ((viewport->Flags & ImGuiViewportFlags_TopMost) ? HWND_TOPMOST
                                                              : HWND_NOTOPMOST)
            : 0;
