...
// while (0)' block so it can be used as a single statement. #define
// IM_ASSERT(_EXPR)  MyAssert(_EXPR) #define IM_ASSERT(_EXPR)  ((void)(_EXPR))
// // Disable asserts
...
// time to
// time after updating to avoid using soon-to-be obsolete function/names.
// #define IMGUI_DISABLE_OBSOLETE_FUNCTIONS #define IMGUI_DISABLE_OBSOLETE_KEYIO
// // 1.87: disable legacy io.KeyMap[]+io.KeysDown[] in favor io.AddKeyEvent().
// This will be folded into IMGUI_DISABLE_OBSOLETE_FUNCTIONS in a few versions.
...
// read comments in imgui_demo.cpp. #define IMGUI_DISABLE // Disable everything:
// all headers and source files will be empty. #define
...
// allocators calling malloc()/free() to avoid linking with them. You will need
// to call ImGui::SetAllocatorFunctions(). #define IMGUI_DISABLE_SSE // Disable
...
// 1-16. (e.g.
// point beyond 0xFFFF like emoticons, dingbats, symbols, shapes, ancient
...
// outside of Dear ImGui sources files. #define IMGUI_STB_TRUETYPE_FILENAME
// "my_folder/stb_truetype.h" #define IMGUI_STB_RECT_PACK_FILENAME
// "my_folder/stb_rect_pack.h" #define IMGUI_STB_SPRINTF_FILENAME
// "my_folder/stb_sprintf.h"    // only used if enabled #define
...
// vcpkg with 'vcpkg install freetype
...
// stb_truetype backend ALONG with the FreeType backend. #define
...
// Read about ImGuiBackendFlags_RendererHasVtxOffset for details. #define
...
// ImDrawList* draw_list, const ImDrawCmd* cmd, void* my_renderer_user_data);
// #define ImDrawCallback MyImDrawCallback

//---- Debug Tools: Macro to break in Debugger
// (use 'Metrics->Tools->Item Picker' to pick widgets with the mouse and break
// into them for easy debugging.) #define IM_DEBUG_BREAK  IM_ASSERT(0) #define
// IM_DEBUG_BREAK  __debugbreak()
