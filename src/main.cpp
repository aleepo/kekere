...
int main(int, char **) {
  ...
  WNDCLASSEXW wc = {sizeof(wc), CS_CLASSDC, WndProc,        0L,     0L, GetModuleHandle(nullptr), nullptr, nullptr,
                    nullptr,    nullptr,    L"KekereClass", nullptr};

  wc.hIcon = wc.hIcon =
      (HICON)LoadImageW(wc.hInstance, L"icon.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_SHARED | LR_DEFAULTSIZE);
  ::RegisterClassExW(&wc);

  //::SetProcessDpiAwarenessContext(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2);
  // #if (WINVER >= 0x0A00) // Windows 10
  //   SetProcessDpiAwarenessContext(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2);
  // #elif (WINVER >= 0x0603 && WINVER < 0x0A00) // 8.1 down to but not including vista
  ...
  HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"Kekere", WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, nullptr, nullptr,
  ...
  DWORD dwret = GetCurrentDirectory(BUFFER_SIZE, Buffer);
  ...
  ImGui::LoadIniSettingsFromDisk(dwret + "\\imgui.ini");
  ...
  while (!done) {
    ...
    // See the WndProc() function below for how  to dispatch events to the Win32
    // backend.
    MSG msg;
