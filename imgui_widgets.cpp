...
#pragma clang diagnostic ignored "-Wformat-nonliteral" // warning: format string is not a string
                                                       // literal // passing non-literal to
                                                       // vsnformat(). yes, user passing incorrect
...
#pragma clang diagnostic ignored "-Wenum-enum-conversion" // warning: bitwise operation between
...
#pragma GCC diagnostic ignored "-Wformat-nonliteral" // warning: format not a string literal, format
                                                     // string not checked
#pragma GCC diagnostic ignored "-Wclass-memaccess"   // [__GNUC__ >= 8] warning: 'memset/memcpy'
                                                   // clearing/writing an object of type 'xxxx' with
                                                   // no trivial copy-assignment; use assignment or
...
bool ImGui::ButtonEx(const char *label, const ImVec2 &size_arg, ImGuiButtonFlags flags) {
  ...
  if ((flags & ImGuiButtonFlags_AlignTextBaseLine) &&
      style.FramePadding.y <
          window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2 &pos, ImGuiDockNode *dock_node) {
  ...
  ImU32 text_col = GetColorU32(ImGuiCol_Text);
  if (hovered || held)
    window->DrawList->AddCircleFilled(bb.GetCenter() + ImVec2(0, -0.5f), g.FontSize * 0.5f + 1.0f,
                                      bg_col, 12);
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2 &size, const ImVec2 &uv0,
  ...
  if (border_col.w > 0.0f) {
    ...
    window->DrawList->AddImage(user_texture_id, bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), uv0,
...
// FIXME: Surprisingly, this seemingly simple widget is adjacent to MANY
// different legacy/tricky layout issues.
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags) {
  ...
  if (flags & ImGuiSeparatorFlags_Vertical) {
    ...
    const float thickness_for_layout =
        (thickness == 1.0f) ? 0.0f : thickness; // FIXME: See 1.70/1.71 Separator()
                                                // change: makes legacy 1-px separator
...
void ImGui::SeparatorTextEx(ImGuiID id, const char *label, const char *label_end, float extra_w) {
  ...
  if (label_size.x > 0.0f) {
    ...
    RenderTextEllipsis(window->DrawList, label_pos,
                       ImVec2(bb.Max.x, bb.Max.y + style.ItemSpacing.y), bb.Max.x, bb.Max.x, label,
...
void ImGui::SeparatorText(const char *label) {
  ...
  // - allow headers to be draggable items (would require a stable ID + a
  // noticeable highlight)
  // - this high-level entry point to allow formatting? (may require ID separate
...
bool ImGui::Combo(const char *label, int *current_item, const char *items_separated_by_zeros,
                  int height_in_items) {
  int items_count = 0;
  const char *p = items_separated_by_zeros; // FIXME-OPT: Avoid computing this, or at
  ...
  if (is_logarithmic) {
    ...
    else if ((v_min * v_max) < 0.0f) // Range crosses zero, so split into two portions
    {
      float zero_point_center =
          (-(float)v_min) /
          ((float)v_max - (float)v_min); // The zero point in parametric space.  There's
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max,
  ...
  if (is_logarithmic) {
    ...
    float t_with_flip = flipped ? (1.0f - t) : t; // t, but flipped if necessary to
...
bool ImGui::SliderBehavior(const ImRect &bb, ImGuiID id, ImGuiDataType data_type, void *p_v,
  ...
  case ImGuiDataType_S16: {
    ImS32 v32 = (ImS32) * (ImS16 *)p_v;
    bool r =
        SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS16 *)p_min,
  ...
  case ImGuiDataType_U16: {
    ImU32 v32 = (ImU32) * (ImU16 *)p_v;
    bool r =
        SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU16 *)p_min,
...
bool ImGui::InputTextWithHint(const char *label, const char *hint, char *buf, size_t buf_size,
                              ImGuiInputTextFlags flags, ImGuiInputTextCallback callback,
                              void *user_data) {
  IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline() or
...
static bool InputTextFilterCharacter(unsigned int *p_char, ImGuiInputTextFlags flags,
  ...
  if (c < 0x20) {
    bool pass = false;
    pass |=
        (c == '\n' && (flags & ImGuiInputTextFlags_Multiline)); // Note that an Enter KEY will emit
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf, int buf_size,
  ...
  if (is_multiline) {
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0)); // Ensure no clip rect so mouse hover
  ...
  if ((init_state && g.ActiveId != id) || init_changed_specs) {
    ...
    state->CurLenA =
        (int)(buf_end - buf); // We can't get the result from ImStrncpy() above because it
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    const bool is_startend_key_down =
        is_osx && io.KeySuper && !io.KeyCtrl && !io.KeyAlt; // OS X style: Line/Text Start and End
                                                            // using Cmd+Arrows instead of Home/End
    ...
    if (apply_edit_back_to_user_buffer) {
      ...
      if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory |
        ...
        if (event_flag) {
          ...
          callback_buf = is_readonly ? buf : state->TextA.Data; // Pointer may have been invalidated
  ...
  if (render_cursor || render_selection) {
    ...
    if (render_cursor && state->CursorFollow) {
      ...
      if (is_multiline) {
        ...
        draw_pos.y += (draw_window->Scroll.y - scroll_y); // Manipulate cursor pos immediately avoid
                                                          // a frame of lag
    ...
    if (is_multiline)
      text_size =
          ImVec2(inner_size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_display_end) *
...
bool ImGui::ColorEdit4(const char *label, float col[4], ImGuiColorEditFlags flags) {
  ...
  if (value_changed && g.LastItemData.ID != 0) // In case of ID collision, the second
...
bool ImGui::ColorPicker4(const char *label, float col[4], ImGuiColorEditFlags flags,
  ...
  ImU32 user_col32_striped_of_alpha =
      ColorConvertFloat4ToU32(ImVec4(R, G, B, style.Alpha)); // Important: this is still including
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags) {
  ...
  IM_ASSERT(
      ImIsPowerOfTwo(flags & ImGuiColorEditFlags_DataTypeMask_)); // Check only 1 option is selected
  IM_ASSERT(
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label,
  ...
  if (display_frame) {
    ...
    if (flags & ImGuiTreeNodeFlags_Bullet)
      ...
      RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y),
...
void ImGui::TreePop() {
  ...
  IM_ASSERT(window->IDStack.Size > 1); // There should always be 1 element in the IDStack
                                       // (pushed during window creation). If this
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char *label,
  ...
  if (values_count >= values_count_min) {
    ...
    float t0 = 0.0f;
    ImVec2 tp0 = ImVec2(
        t0, 1.0f - ImSaturate((v0 - scale_min) * inv_scale)); // Point in the normalized space of
                                ...
                                // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
  {
    ...
    if (menu_is_open && pressed && menuset_is_open) // Click an open menu again to close it
    {
      ...
      want_open = menu_is_open = false;
               ...
               (hovered && menuset_is_open && !menu_is_open)) // First click to open, then
...
bool ImGui::BeginTabBarEx(ImGuiTabBar *tab_bar, const ImRect &tab_bar_bb, ImGuiTabBarFlags flags,
  ...
  // Draw separator
  const ImU32 col = GetColorU32((flags & ImGuiTabBarFlags_IsFocused) ? ImGuiCol_TabActive
...
void ImGui::TabBarAddTab(ImGuiTabBar *tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow *window) {
  ...
  IM_ASSERT(g.CurrentTabBar != tab_bar); // Can't work while the tab bar is active as our tab
                                         // doesn't have an X offset yet, in theory we could/should
                                         // test something like (tab_bar->CurrFrameVisible <
                                         // g.FrameCount) but we'd need to solve why triggers the
                                         // commented early-out assert in BeginTabBarEx() (probably
                                         // dock node going from implicit to explicit in same frame)
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar *tab_bar, ImGuiID tab_id,
  ...
  ImGuiContext &g = *GImGui;
  float margin = g.FontSize * 1.0f; // When to scroll to make Tab N+1 visible always make
...
bool ImGui::BeginTabItem(const char *label, bool *p_open, ImGuiTabItemFlags flags) {
  ...
  IM_ASSERT((flags & ImGuiTabItemFlags_Button) == 0); // BeginTabItem() Can't be used with button
...
bool ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open,
  ...
  if (g.DragDropActive &&
      !g.DragDropPayload.IsDataType(IMGUI_PAYLOAD_TYPE_WINDOW)) // FIXME: May be an opt-in property
